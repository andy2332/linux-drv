...
DEFINE_TIMER(testtimer,timer_func,0,0);
...
static void timer_func(unsigned long data)
{
	printk(KERN_INFO "time out %s %s line %d\n", __FILE__, __FUNCTION__, __LINE__);
...
static irqreturn_t test_key(int irq,void *args)
{
	...
	mod_timer(&testtimer,jiffies + msecs_to_jiffies(20));
	return IRQ_HANDLED;
}
...
static int led_drv_probe(struct platform_device *pdev)
{
	...
	//testtimer.expires = jiffies + msecs_to_jiffies(50);
