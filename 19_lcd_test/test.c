...
static int cursor_bitmap_format_convert(char *dst,char *src)
{
    ...
    char *pdst = dst;
    char *p = psrc;
	printf("start cursor_bitmap_format_convert\n");
    /* 由于bmp存储是从后面往前面，所以需要倒序进行转换 */
    //pdst += (width * height * 3);
	pdst = pdst + (width * height * 3);
	printf("cursor_bitmap_format_convert : width:%d,height:%d\n",width,height);
    for(i=0;i<height;i++){
        p = psrc + (i+1) * width * 3;
        for(j=0;j<width;j++){
            pdst -= 3;
	...
	printf("end cursor_bitmap_format_convert\n");
    return 0;
}
